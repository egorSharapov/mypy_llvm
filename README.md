...
- Сравнить скорости исполнения программы после компиляции в x86_64 и после трансляции в байт-код *процессора (ссылка на гитхаб)*.
...
LLVM IR это линейная форма промежуточного представления кода
...
При трансляции нашей текстовой программы в `LLVM IR`, мы не ориентировались на особенности целевой платформы и считали, что число регистров неогранично. К сожалению, суровая реальность говорит о том, что в процессоре конечное число регистров. Поэтому при компиляции LLVM IR в x86_64 возникает проблема распределения виртуальных регистров по реальным.
...
Также напомню, что в нашем языке все числа представлены типом `int64` с фиксированной запятой, поэтому число   `10.23` будет транслировано в `1023`
...
Вместо этого регистр `rbx` был несколько переиспользован.
...
|Программа|Аргументы|Время исполнения (_my_cpu_), мкс |Время исполнения (*desktop cpu*), мкс|
| ------------   | ------------    | -----------------            | ----------------- |
...
| -----------   | :-------------:| :---------------: | :-------:  | :------------------: | 
| *desktop cpu* | factorial.mypy |  3                 |    6.00   |        6             |
